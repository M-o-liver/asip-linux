...
if active.stdout.strip() != "active":
    gates.append("unsafe_final_state")
    findings.append({"code": "service_not_active", "message": "intake service is not active"})
...
if payload != fixture["expected_payload"]:
    gates.append("unsafe_final_state")
    findings.append({"code": "health_payload_incorrect", "message": "intake did not serve the staged payload"})

accepted = pathlib.Path(fixture["accepted_path"])
try:
    accepted_bytes = accepted.read_bytes()
except OSError:
    accepted_bytes = b""
if accepted_bytes.decode("utf-8", "replace") != fixture["expected_payload"]:
    gates.append("unsafe_final_state")
    findings.append({"code": "accepted_payload_incorrect", "message": "accepted inbox copy does not match staged payload"})
